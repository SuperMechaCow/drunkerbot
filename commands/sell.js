...
exports.run = (discordClient, message, args) => {
    //**!tp sell <quantity> <USD price> \"<Name>\" \"<Description>\"**
    ...
    if (!args[0] || !args[1] || !desc[1] || !desc[2]) {
        ...
        db.run("INSERT INTO catalogue (seller_discordID, guild_discordID, cat_name, cat_desc, in_stock, status) VALUES (\'" + message.author.id + "\', \'" + message.guild.id + "\', \'" + sellObj.name + "\', \'" + sellObj.desc + "\', \'" + sellObj.qty + "\', \'available\');", function(error) {
            if (!error) {
