...
function checkTimes() {
	db.all(`SELECT * FROM t_playtest WHERE status = 'accepted';`, function(error, results) {
		if (results != undefined) {
			results.forEach(function(result, index) {
				...
				if (moment.unix(result.time).isSameOrBefore(moment())) {
					...
					db.get(`SELECT * FROM t_playtest WHERE status = 'active';`, function(getError, getResults) {
						if (getError) {
							logger.error(getError);
						} else if (getResults != undefined) {
							db.run(`UPDATE t_playtest SET status = 'canceled' WHERE id = ${result.id};`, function(runError) {
								if (runError) logger.error(runError);
							});
							//Send error
						} else {
							db.run(`UPDATE t_playtest SET status = 'active' WHERE id = ${result.id};`, function(runError) {
								if (runError) {
